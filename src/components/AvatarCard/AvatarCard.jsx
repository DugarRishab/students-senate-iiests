const AvatarCard = ({
  ...
  const gradient = `
      ...
      `;
  return (
    <div className="flex flex-col items-center max-w-xs mx-auto text-center p-4 md:p-0">
      <div
      ...
      >
        <div
          className={`relative rounded-full overflow-hidden size-28 md:size-32 ${bgColor}`}
        >
          {profilePic ? (
            <img
              src={profilePic}
      ...
      <h2 className="mt-4 text-lg md:text-xl font-bold">{name}</h2>
      {/* Divider */}
      <div className="w-32 h-0.5 bg-gray-600 my-2"></div>
      <p className="text-center text-xs md:text-base max-w-xs">
        Department of <br />
        {department}
      </p>
    </div>
